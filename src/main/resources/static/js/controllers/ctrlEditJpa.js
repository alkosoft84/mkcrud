(function() {
    ...
    angular.module('playerModule').controller('ctrlEditJpa', ['$scope', '$http', '$routeParams', '$location', 'ngToast', function ($scope, $http, $routeParams, $location, ngToast) {
        $scope.db = "JPA";
        if ($routeParams.itemId != null) {
            $http.get('http://localhost:8080/getplayerjpa/' + $routeParams.itemId).success(function (player) {
                $scope.player = player;
        ...
        $scope.onSubmit = function () {
            $http({
                url: 'http://localhost:8080/editplayerjpa',
                dataType: 'json',
                method: 'POST',
                data: $scope.player,
                headers: {
